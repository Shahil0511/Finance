...
export default function ReturnsReportTable() {
  const { filters, setPage, setSort, setFilters } = useReturnsFilterStore();
  const params = cleanParams(filters);
  const { data: res, isLoading, isFetching, isError, error, refetch } = useGetReturnsListQuery(params);
  const { exportCsv, exporting } = useCsvExport();

  const actions = RETURNS_REPORT.exports.map((exp) => (
    <Button
      key={exp.endpoint}
      variant="outline"
      size="sm"
  ...
  return (
    <DataTable
      title={RETURNS_REPORT.tableTitle}
      columns={RETURNS_REPORT.columns}
      data={res?.data ?? []}
