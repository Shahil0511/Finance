...
const CARDS = [
  { key: 'total_returns',       label: 'Total Returns',       Icon: RotateCcw,   color: 'text-violet-500',  fmt: formatNumber   },
...
export default function ReturnsSummaryCards() {
  const { filters } = useReturnsFilterStore();
  const { data, isLoading, isFetching, isError, error, refetch } = useGetReturnsSummaryQuery(cleanParams(filters));
  ...
  if (isError) {
    return (
      <ErrorCard compact message={error?.data?.detail || 'Could not load summary'} onRetry={refetch} />
    );
  }
