...
export default function SalesFiltersPanel() {
  ...
  const { data: opts, isLoading, isError, refetch } = useGetSalesFiltersQuery();
  ...
  // Has the draft diverged from the currently applied filters?
  const hasChanges = FILTER_KEYS.some((k) => draft[k] !== (filters[k] ?? ''));
  // Are any non-date filters active in the applied state?
  ...
  const clear = () => {
    reset();
    // Sync draft to the freshly-reset store state
    const newFilters = useSalesFilterStore.getState().filters;
              ...
              {isLoading ? (
                ...
                <>
                  ...
                  <FilterSelect
                    ...
                    onChange={patch("orderStatus")}
                    options={opts?.orderStatuses ?? []}
                  />
                  <FilterSelect
                    label="Warehouse"
                    value={draft.warehouse}
                    onChange={patch("warehouse")}
                    options={opts?.warehouses ?? []}
                  />
                  <FilterSelect
                    label="Payment"
                    value={draft.paymentType}
                    onChange={patch("paymentType")}
                    options={opts?.paymentTypes ?? []}
                  />
                  <FilterSelect
                    label="State"
                    value={draft.state}
