...
export default function SalesSummaryCards() {
  ...
  const { data, isLoading, isFetching, isError, error, refetch } = useGetSalesSummaryQuery(cleanAggParams(filters));

  if (isError) {
    return <ErrorCard compact message={error?.data?.detail || 'Could not load summary'} onRetry={refetch} />;
  }

  const cards = SALES_REPORT.cards;
  const grid = cards.length >= 4
    ? 'grid-cols-2 sm:grid-cols-3 xl:grid-cols-5'
    : 'grid-cols-1 sm:grid-cols-2 xl:grid-cols-4';

  return (
    <section aria-label={`${SALES_REPORT.title} summary`} className={cn('relative grid gap-3 sm:gap-4', grid)}>
