...
const CARDS = [
  { key: 'total_orders', label: 'Total Rows', Icon: ShoppingCart, color: 'text-blue-500', fmt: formatNumber },
...
export default function TataCliqSalesSummaryCards() {
  const { filters } = useTataCliqSalesFilterStore();
  const { data, isLoading, isFetching, isError, error, refetch } =
    useGetTataCliqSalesSummaryQuery(cleanParams(filters));

  if (isLoading) {
    return <div className="card px-4 py-3"><LoadingBanner /></div>;
  }
