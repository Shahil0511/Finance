...
const FILTER_KEYS = ['dateFrom','dateTo','returnStatus','qcStatus','search'];
...
export default function TataCliqReturnsFiltersPanel() {
  ...
  const { data: opts, isLoading, isError, refetch } = useGetTataCliqReturnsFiltersQuery();
  ...
  const clear = () => {
    reset();
    const newFilters = useTataCliqReturnsFilterStore.getState().filters;
    setDraft(Object.fromEntries(FILTER_KEYS.map((k) => [k, newFilters[k] ?? ''])));
  };

  return (
    <div className="card overflow-hidden">
      <button onClick={() => setOpen((o) => !o)} className="w-full flex items-center justify-between px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
        ...
        {open && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden border-t border-slate-200 dark:border-slate-800">
            ...
            <div className="px-4 py-4 flex flex-wrap gap-3">
              <FilterInput label="Processed Time" type="date" value={draft.dateFrom} onChange={patch("dateFrom")} />
              ...
              {isLoading ? (
                ...
                <>
                  <FilterSelect label="Return Status" value={draft.returnStatus} onChange={patch("returnStatus")} options={opts?.returnStatuses ?? []} />
                  <FilterSelect label="QC Status" value={draft.qcStatus} onChange={patch("qcStatus")} options={opts?.qcStatuses ?? []} />
                </>
              )}
