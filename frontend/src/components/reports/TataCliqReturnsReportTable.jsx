...
export default function TataCliqReturnsReportTable() {
  const { filters, setPage, setSort, setFilters } = useTataCliqReturnsFilterStore();
  const params = cleanParams(filters);
  const { data: res, isLoading, isFetching, isError, error, refetch } = useGetTataCliqReturnsListQuery(params);
  const { exportCsv, exporting } = useCsvExport();

  const actions = TATA_CLIQ_RETURN_REPORT.exports.map((exp) => (
    <Button
      key={exp.endpoint}
      variant="outline"
      size="sm"
  ...
  return (
    <DataTable
      title={TATA_CLIQ_RETURN_REPORT.tableTitle}
      columns={TATA_CLIQ_RETURN_REPORT.columns}
      data={res?.data ?? []}
      ...
      rowKey={TATA_CLIQ_RETURN_REPORT.rowKey}
    />
  );
}
